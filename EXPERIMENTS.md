...
Source: the configuration used for the paper's Section 6 computational study.

This document lists the committed data and computational studies used in Section 6.
...
The committed datasets use the following English-only layout:
...
The large health and insole CSV files are stored with Git LFS. The repository
does not include the cohort-generation implementation.
...
| S6-E5 | Files under `src/s6_e5_similarity_weighted_topk/` | `scripts/run_s6_e5_similarity_weighted_topk.sh` | `results/s6_e5_similarity_weighted_topk` |
