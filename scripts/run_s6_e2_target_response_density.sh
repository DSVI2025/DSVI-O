...
OUT_ROOT="${OUT_ROOT:-${STABLE_DIR}/results/s6_e2_target_response_density}"
FULL_BASELINE_ROOT="${FULL_BASELINE_ROOT:-${STABLE_DIR}/results/s6_e2_target_full_recomputation}"
DEVICE="${DEVICE:-cpu}"
...
RESPONSE_STRIDES="${RESPONSE_STRIDES:-120,240,720,1440}"
...
if ! mat_complete "${DATA_DIR}"; then
  echo "Missing S6-E2a MAT input files under DATA_DIR=${DATA_DIR}" >&2
...
cmd=(
  "${PYTHON_BIN}" "${STABLE_DIR}/src/s6_e2_target_response_density/run_target_observation_density_baseline.py"
  --code-root "${STABLE_DIR}/src/s6_e2_target_full_recomputation"
  --data-dir "${DATA_DIR}"
  --out-root "${OUT_ROOT}"
  --full-baseline-root "${FULL_BASELINE_ROOT}"
  --user-ids "${USER_IDS}"
  --response-strides "${RESPONSE_STRIDES}"
...
if [[ -n "${TIME_STEP_LIMIT:-}" ]]; then
  cmd+=(--time-step-limit "${TIME_STEP_LIMIT}")
fi
